...
+ (UIImage*)thumbnailForFileWithData:(NSData*)imgData _thumW:(CGFloat)thumW{
    ...
    imageSource = CGImageSourceCreateWithData((__bridge CFDataRef)imgData, nil);
    if (imageSource == NULL)
    {
        return nil;
    }
    ...
    NSDictionary* thumbnailInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                   ...
                                   [NSNumber numberWithInt:thumW], kCGImageSourceThumbnailMaxPixelSize,
                                   nil];
    //create thumbnail picture
    thumbnail = CGImageSourceCreateThumbnailAtIndex(imageSource, 0, (__bridge CFDictionaryRef)thumbnailInfo);
    UIImage*    img = [[UIImage alloc] initWithCGImage:thumbnail scale:1.0 orientation:UIImageOrientationUp];
    CFRelease(thumbnail);
    CFRelease(imageSource);
    return img;
